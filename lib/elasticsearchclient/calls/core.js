...
ElasticSearchClient.prototype.deleteDocument = function(indexName, typeName, documentId, options) {
  ...
  return this.query(path, options, 'DELETE');
};
...
ElasticSearchClient.prototype.search = function(indexName, typeName, queryObj, options) {
  ...
  // Make a nice call if the typeName is missing or set to blank
  // http://server:9200/graylog2/_search instead of http://server:9200/graylog2//_search
...
ElasticSearchClient.prototype.multiget = function(indexName, typeName, documentArray, options) {
  ...
  path += '/_mget';
  if (objArgs[1]) path += "?" + querystring.stringify(objArgs[1]);

  var data = {docs: []};
  for (var j = 0; j < objArgs[0].length; j++) {
    data.docs.push(objArgs[0][j]);
  }
